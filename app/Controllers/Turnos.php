...
class Turnos extends BaseController
{
     ...
     public function guardar()
    {
        ...
        if (!$this->turnoModel->verificarDisponibilidad($quirofanoId, $fecha, $horaInicio, $duracion)) {
            // Guardar los datos del procedimiento para mantenerlos en el formulario
            $procedimientoData = [
                'procedimiento' => $this->request->getPost('procedimiento'),
                'otro_procedimiento' => $this->request->getPost('otro_procedimiento')
            ];
            
            return redirect()->back()
                ->withInput()
                ->with('error', 'El quirófano no está disponible en el horario seleccionado')
                ->with('procedimiento_data', $procedimientoData);
        }
